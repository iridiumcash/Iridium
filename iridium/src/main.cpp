...
uint256 hashGenesisBlock("0x00000e6422009a4502496c783aadc78b01eb51d004d492220dfd8928b283c964");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Iridium: starting difficulty is 1 / 2^12
...
{
	// Iridium: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 60; // Iridium: 1 minute
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
	...
	// Iridium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
	...
	// was causing -reindex to crash Iridiumd
	if (hash != hashGenesisBlock) {
...
{
	// Iridium: temporarily disable v2 block lockin until we are ready for v2 transition
	return false;
...
bool LoadBlockIndex()
{
	if (fTestNet)
	{
		...
		hashGenesisBlock = uint256("0x00000e6422009a4502496c783aadc78b01eb51d004d492220dfd8928b283c964");
	}
...
bool InitBlockIndex() {
	...
	if (!fReindex) {
		...
		const char* pszTimestamp = "08/08/2017 - Rosetta heads for space Rubber Duck";
		CTransaction txNew;
		txNew.vin.resize(1);
		txNew.vout.resize(1);
		txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
		txNew.vout[0].nValue = 100 * COIN;
		txNew.vout[0].scriptPubKey = CScript() << ParseHex("043184710fa689ad5023690c80f3a49c8f13f8d45b88857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
		...
		printf("%s\n aa hash che", hash.ToString().c_str());
		printf("%s\n aa genesis hash che", hashGenesisBlock.ToString().c_str());
		printf("%s\n MerkleRoot ", block.hashMerkleRoot.ToString().c_str());

		// Merkle root
		assert(block.hashMerkleRoot == uint256("0x3b07cc6eb24502f40237407f01cbd0eb5629f663d18b6bd3b6a96ebe49fa5e37"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc2, 0xb3, 0xee };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	// Found a solution
	{
		...
		if (!ProcessBlock(state, NULL, pblock))
			return error("IridiumMiner : ProcessBlock, block not accepted");
	}

	return true;
}

void static IridiumMiner(CWallet *pwallet)
{
	printf("IridiumMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("Iridium-miner");
	...
	try {
	 ...
	 while (vNodes.empty())
		...
		printf("Running IridiumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
	...
	{
		printf("IridiumMiner terminated\n");
		throw;
	}
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
	...
	for (int i = 0; i < nThreads; i++)
		minerThreads->create_thread(boost::bind(&IridiumMiner, pwallet));
}
